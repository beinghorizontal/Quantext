...
from plotly.offline import plot
from bt_extra_eod import btextra, equity_curve_plot, backtest_plot

from bACKTEST_COMPARE import df_mp_daily, backtest, short_condition
...
df_daily['Low_5'] = df_daily['Low'].rolling(5).min()
...
short_condition = ("(df_daily['Low'] < df_daily['Low_5'].shift(1)) &"
"(df_daily['AR_Small'] > df_daily['AR_Long'])")

df_daily['Long'] = np.where(eval(long_condition), 1, 0)
df_daily['Short'] = np.where(eval(short_condition), -1, 0)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.position.is_short:
                if(self.data.Low[-1] < self.entry_price_sell*(1-pt_div)
                or(self.data.High[-1] > self.entry_price_sell *(1+sl_div))
                ):
                    self.position.close()
...
stats_extra = btextra(df_daily, trades.copy())
print(stats_extra[0])
